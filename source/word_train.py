...
########## LOAD DATA ######################################################
data = pd.read_csv('data/aug_word_data1.csv')
...
num_of_timestamps = 24  # max length of predicted labels #Không đặt trùng vs batch size
...
# Add early stopping
my_callbacks = [
    tf.keras.callbacks.ModelCheckpoint(filepath='model/model_word/'+str(datetime_object)+'/word_model_{epoch:02d}_{val_loss:.2f}_test.h5', 
                                       save_freq='epoch',
                                       monitor='val_loss',
                                       mode='min', 
                                       save_best_only=True,
...
plt.savefig('model/model_word/'+str(datetime_object)+'/word_model_loss_test.png')
plt.show()


with open('model/model_word/'+str(datetime_object)+'/word_model_test.txt', 'w', encoding='utf-8') as f:
...
with open('model/model_word/'+str(datetime_object)+'/word_model_test.txt', 'a', encoding='utf-8') as f:
