...
if not os.path.exists('model\model_word/'+ str(datetime_object)):
    os.mkdir('model\model_word/'+ str(datetime_object))
...
test_cer_names, test_wer_names = [], []
test_cer_MSSV, test_wer_MSSV = [], []
test_cer_diem, test_wer_diem = [], []
image_size = 0
acc = 0
uncorrect = []
############## RESTORE MODEL ##########
...
for filename in os.listdir("data/sample"):
    ...
    pr = name_recognized.strip() #strip xóa kí tự trắng 2 đầu
    #pr = pr.lower()
    tr = 'Mai Chí Bảo'
    #tr = tr.lower() #Biểu thị cho việc hậu xử lý sau này. Sẽ lower case toàn bộ danh sách tên

    test_cer_names.append(cer(tr, pr))
    test_wer_names.append(wer(tr, pr))
    ...
    #MSSV_index, MSSV_recognized = lexicon_search (MSSV_recognized.strip(), MSSV_list)


    pr = MSSV_recognized.strip() #strip xóa kí tự trắng 2 đầu
    tr = '1710586'
    
    test_cer_MSSV.append(cer(tr, pr))
    test_wer_MSSV.append(wer(tr, pr))
    ...
    if str(name_MSSV_recognized) == 'Mai Chí Bảo 1710586':
        acc += 1    
        print ('Name & MSSV Accuracy:%.2f%%' %(acc*100/image_size))
    else:
        uncorrect.append(filename)
    ...
    test_cer_diem.append(cer(tr, pr))
    test_wer_diem.append(wer(tr, pr))
...
test_cer_MSSV, test_wer_MSSV
avg_cer_names = sum(test_cer_names)/len(test_cer_names)   
avg_wer_names = sum(test_wer_names)/len(test_wer_names)

avg_cer_MSSV = sum(test_cer_MSSV)/len(test_cer_MSSV)   
avg_wer_MSSV = sum(test_wer_MSSV)/len(test_wer_MSSV)

avg_cer_diem = sum(test_cer_diem)/len(test_cer_diem)   
avg_wer_diem = sum(test_wer_diem)/len(test_wer_diem)

print ('\nAverage CER names: %.2f%%' %(avg_cer_names*100))
print ('Average WER names: %.2f%%' %(avg_wer_names*100))

print ('\nAverage CER MSSV: %.2f%%' %(avg_cer_MSSV*100))
print ('Average WER MSSV: %.2f%%' %(avg_wer_MSSV*100))

print ('\nAverage CER diem: %.2f%%' %(avg_cer_diem*100))
print ('Average WER diem: %.2f%%' %(avg_wer_diem*100))

with open('model\multi_digit_model/2021-10-30\digit_model_2021-10-30.txt', 'a', encoding='utf-8') as f:
    
    f.writelines('\n--------------------------\nResult on Main_{}'.format(image_size))
    f.writelines ('\nword_model_dir: {}'.format(word_model_dir))
    f.writelines ('\ndigit_model_dir: {}'.format(digit_model_dir))

    f.writelines('\n\nAverage CER names: %.2f%%' %(avg_cer_names*100))
    f.writelines('\nAverage WER names: %.2f%%' %(avg_wer_names*100))
    
    f.writelines('\n\n\nAverage CER MSSV: %.2f%%' %(avg_cer_MSSV*100))
    f.writelines('\nAverage WER MSSV: %.2f%%' %(avg_wer_MSSV*100))

    f.writelines('\n\n\nAverage CER diem: %.2f%%' %(avg_cer_diem*100))
    f.writelines('\nAverage WER diem: %.2f%%' %(avg_wer_diem*100))
